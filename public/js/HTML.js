...
async function InitImages(){
	...
	logo.src = await downloadImage("/icon.png");
	jordan.src  = await downloadImage("/tilting.png");
	rushShirt.src  = await downloadImage("/topLeft.png");
	video.src = await downloadImage("/video.mp4");
	triGraphic.src = await downloadImage("/bottom.png");
	rushInfo.src = await downloadImage("/rushEvents.png");

	let siteDB = await getData("site");

	for (var i = 0; i < siteDB.length; i++) {
		if(siteDB[i].primaryColor){
			primaryColor = siteDB[i].primaryColor;
		}
		if(siteDB[i].secondaryColor){
			secondaryColor = siteDB[i].secondaryColor;
		}
		if(siteDB[i].background){
			background = siteDB[i].background;
		}
	}
...
function InitLogo(){
	...
	let currentWidth = Math.max(screen.width, MAX_SCREEN_WIDTH);
...
function sendUserData(){
	let data = {
				...
				Insta:document.getElementById("formInsta").value, 
	...
	for (var i = 0; i < forms.length; i++) {
		if(forms[i].value == ""){
			document.getElementById("sumbitResult").textContent = "Please Fill in All Input Feilds";
			return; 
		}
	}

	console.log(submissionTries)
	if(submissionTries > 1){
		document.getElementById("sumbitResult").textContent = "Maximum Submissions";
		return;
	}

	let response = sendData(data);
	...
	document.getElementById("sumbitResult").textContent = "You've Lost Connnection To Internet";
}
